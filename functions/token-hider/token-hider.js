...
exports.handler = async function (event, context, callback) {
  ...
  if (NODE_ENV === 'development') {
    URL = `${API_URL}${COLLECTION_ID}/photos?client_id=${API_KEY}&per_page=${paginationCap}&page=1`
  }
  if (NODE_ENV === 'production') {
    URL = `${process.env.UNSPLASH_URL}${COLLECTION_ID}/photos?client_id=${process.env.UNSPLASH_TOKEN}&per_page=${paginationCap}&page=1`
  }
